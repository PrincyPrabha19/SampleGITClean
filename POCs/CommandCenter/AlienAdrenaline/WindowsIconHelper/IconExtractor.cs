...
namespace AlienLabs.WindowsIconHelper
{
    ...
    public class IconExtractor : IDisposable
    {
        ...
        {
            if (!this.IconCache.ContainsKey(id))
                this.IconCache[id] = GetIconFromLibById(id);
        ...
        /// <summary>
        /// Gets a System.Drawing.Icon that represents RT_GROUP_ICON with a given id from the executable module.
        /// </summary>
        /// <param name="id">The index of the RT_GROUP_ICON in the executable module.</param>
        /// <returns>Returns System.Drawing.Icon.</returns>
        private Icon GetIconFromLibById(int id)
        {
            var resourceName = (from r in this.IconNamesList
                                where r.Id == id
                                select r).FirstOrDefault();

            byte[] resourceData = GetResourceData(this.ModuleHandle, resourceName, ResourceTypes.RT_GROUP_ICON);
            //Convert the resouce into an .ico file image.
            using (MemoryStream inputStream = new MemoryStream(resourceData))
            using (MemoryStream destStream = new MemoryStream())
            {
