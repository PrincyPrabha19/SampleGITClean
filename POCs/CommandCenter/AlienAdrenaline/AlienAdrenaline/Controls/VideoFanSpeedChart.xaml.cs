...
{
    public partial class VideoFanSpeedChart
    {
        public override bool IsMainChart { get { return false; } }

        #region Constructor
        public VideoFanSpeedChart(PerformanceMonitoringService monitorService, int chartSeconds, bool isRealtimeChart)
        {
            InitializeComponent();
            ...
            var infoResults = monitorService.VideoInfoResults.Where(x => x.CategoryInfo == MonitoringCategoryInfo.Video_FanSpeed).ToList();
            RefreshLayout(infoResults, panelCharts);
        }

        public VideoFanSpeedChart(List<ChartData> chartData1, List<ChartData> chartData2, int chartSeconds, bool isRealtimeChart)
        {
            InitializeComponent();
