...
namespace ManagedCustomActions
{
    ...
    public class CustomAction
    {
        ...
        private const string NOTIFICATIONAPP = "AWCC.ToastNotifier.exe";
        ...
        public UInt32 OnStaged(object pDispatch)
        {
            ...
            try
            {
                ...
                suiteExtension = GetExtensionFromDispatch(pDispatch);
                // logic is changed not to abort but delete the package which is not signed
                if (false == IsPackageSigned(suiteExtension))
                {
                    
                    suiteExtension.LogInfo("Either package is not completely downloaded or Package is corrupted");
                }
                suiteExtension.LogInfo("Percentage = " + progress.ToString());
